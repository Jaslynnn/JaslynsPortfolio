...
1. **Gloo Communications tooling case study** — currently missing from the site entirely, needs to be written from scratch. Company named explicitly here. **This is the homepage lead. Gets a new "How it was made" pop-up, built as part of this refresh — pop-up content is the detailed breakdown below (origin story, per-tool build status, the website rebuild detail, and the resources/credibility note).**
   ...
   **Detail pulled from the internship report (source of truth for this case study — more accurate than earlier draft summaries):**
   - **Origin story:** Gin (founder) asked if client timelines could look "nicer," expecting something simple like a Canva template. Jaslyn built an actual generative tool instead. Subsequent tools came about informally — team members brought her their own workflow problems once they saw the Timeline Builder in use.
   - **Timeline Builder** (built/complete) — project-facing tool where an account lead enters tasks, owners, and dates for a single client project; generates a client-ready Gantt PDF plus a CSV export. Built as a self-contained HTML file so all data stays on the user's own computer — a deliberate choice for client-data privacy.
   - **Internal Timeline** (built/complete) — imports CSV exports from any number of project timelines into one combined, sortable, cross-project view for the team, without centralizing raw client data. Same self-contained HTML approach as Timeline Builder, for the same data-locality reason.
   - **Gloo Mail / Mergo** (status: in progress as of last known update — confirm current status with Jaslyn before publishing) — personalized mass-email tool for client and media communications, addressing local sync and bounce-tracking pain points.
   - **Coverage Report Automation** (status: in progress) — intended to reduce the manual screenshot-and-format cycle behind client coverage decks.
   - **Gloo Portal** (concept stage) — a consolidated internal chatbot-style interface for admin functions like leave applications. Worth a brief mention as evidence of forward-thinking scope, not a completed deliverable.
   - **Website:** not a from-scratch build — a WordPress-to-static rebuild of gloopr.biz, done wireframe-first, content-first, via a PDF-crop workflow. Now live at high fidelity. Describe it accurately as a rebuild/migration in this case study (the simplified "built a website" phrasing is fine for the bio, but this case study should be precise).
   - **AI tooling used throughout for prototyping/drafting where it genuinely sped up iteration** — matches the "Custom AI Workflows" line already in the resume skills section.
   ...
   **Do not overstate completion status** — Timeline Builder and Internal Timeline are the two tools that are actually finished and demoable; the others are honestly in-progress or conceptual. A case study that's precise about status is more credible than one that implies everything shipped.
