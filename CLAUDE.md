...
**Home** — portfolio-first landing. The hero (Homewrecker's video, title text, positioning line) stays exactly as currently composed — see Design Constraint above. Below the hero, a **"Featured Work"** grid shows a curated 3 (`content/featured.ts` — PR Agency Website, PR Workflow Tooling, Homewrecker) with a "View More" link to `/work`.
